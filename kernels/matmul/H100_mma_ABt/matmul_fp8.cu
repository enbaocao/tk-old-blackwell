...
struct matmul_layout_fp8 {
    using  a_tile         = st_fl8_e4m3<64, 64>;
    using  b_tile         = st_fl8_e4m3<64, 64>;
    ...
    struct input_block    { a_tile a[M_BLOCK]; b_tile b[N_BLOCK]; };
    struct finish_block   { c_tile c[M_BLOCK][N_BLOCK]; };
...
struct matmul_template_fp8 {
    ...
    __device__ static inline void common_setup(common_setup_args<layout> args) {
        ...
        args.num_iters = args.globals.A.cols()/64;  // K tiling by 64
...
void cpu_gemm(float* a, float* b, float* c, int M, int N, int K) {
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            float sum = 0.0f;
            for (int k = 0; k < K; k++) {
                sum += a[i * K + k] * b[j * K + k]; // ABt: B treated as KxN row-major
            }
            c[i * N + j] = sum;
        }
    }
