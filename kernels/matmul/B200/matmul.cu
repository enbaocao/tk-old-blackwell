...
void matmul(const __grid_constant__ matmul_globals g) { // clustered kernel with 2 CTAs per cluster, fixed threads per CTA
     ...
     *   - Consumer WGs (warpgroupid in [0, NUM_CONSUMERS)) drain the accumulators and commit results to global D via TMA stores.
    ...
    if(warpgroupid == NUM_CONSUMERS) { // producer warpgroup: loads A/B tiles and performs MMA to fill accumulators
        ...
        if(warpgroup::warpid() == 3) { // dedicated loader warp for TMA transfers
             ...
             */
            d_tt_t d_tt = tm_alloc.allocate<d_tt_t>(warpgroup::warpid()*Nb); // allocate accumulator tile in tensor memory
    ...
    else { // consumer warpgroups: read accumulators and commit to global D
        /*
         * Consumers
         * - Wait on outputs_arrived parity for tile t, then stream the Mb×Nb accumulator tile from tensor memory in 64‑column strips.
         * - Each strip: load into registers (rt_bf) → stage into shared (d_smem) → issue TMA store_async to global D.
        ...
        d_tt_t d_tt = tm_alloc.allocate<d_tt_t>(warpgroupid*Nb); // their view into tensor memory accumulators
        for(int task_iter = 0; true; task_iter++) { // for each tile assignment
            ...
            rt_bf<Mb/4, d_tile::cols> d_reg[4]; // register tiles to fetch strips of d_tt
            ...
            for(int i = 0; i < Nb/d_tile::cols; i++) { // iterate 64-wide columns across Nb
                warpgroup::load_async(d_reg[i], d_tt.subtile<tt<float, 128, 64>>(0, 64*i)); // async load subtile into registers
            }
            ...
            warpgroup::store(d_smem, d_reg[0]); // stage first strip to shared for TMA store
...
void cpu_gemm(float* a, float* b, float* c, int M, int N, int K) { // simple CPU reference GEMM: C = A * B^T (B is stored row-major KxN)
...
void inner_run(bf16 *d_A, bf16 *d_B, bf16 *d_C, size_t M, size_t N, size_t K, dim3 grid, dim3 block) { // one kernel launch wrapper
    ...
    matmul<<<grid, block, MAX_SHARED_MEMORY-1024>>>(G); // launch with near-max dynamic shared memory (leave headroom)
}
...
int run_benchmark(size_t M, size_t N, size_t K) { // allocate, initialize, run kernel, validate, and report perf
    ...
    cudaFuncSetAttribute(matmul, cudaFuncAttributeMaxDynamicSharedMemorySize, mem_size); // opt-in to large dynamic smem
...
int main() { // standalone driver
    int N; // square sizes to test
    // N = 1024; // uncomment to run smaller sizes
    // run_benchmark(N, N, N);
    // N = 2048;
    // run_benchmark(N, N, N);
    // N = 4096;
