...
void fp8_matmul_kernel(const __grid_constant__ matmul_globals g) {
...
void inner_run(fp8e4m3 *d_A, fp8e4m3 *d_B, half *d_C, size_t M, size_t N, size_t K, dim3 grid, dim3 block) {
    ...
    globals G{Ag, Bg, Dg};
    fp8_matmul_kernel<<<grid, block, MAX_SHARED_MEMORY-1024>>>(G);
}
...
int run_benchmark(size_t M, size_t N, size_t K) {
    ...
    cudaFuncSetAttribute(fp8_matmul_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    ...
    for (int i = 0; i < M * N; ++i) {
        ...
        if(error > 1.0) { // tolerant due to fp8 numerics
